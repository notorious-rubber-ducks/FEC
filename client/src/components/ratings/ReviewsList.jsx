  ...
  return (
    <div>
      ...
      <div className="scrollable" style={{ overflowY: 'scroll', maxHeight: 500, paddingRight: 8 }}>
        ...
        {filtered
          ? filtered.map((review) => <ReviewBody props={review} />)
          : reviewDataProps
            ? (reviewList
              ? reviewList.slice(0, sliceIndex).map((review) => <ReviewBody props={review} />) : reviewDataProps.results.slice(0, sliceIndex).map((review) => <ReviewBody props={review} />)) : null}
      </div>
      <div style={{ marginTop: '2%' }}>
        {listSize > 2 ? <button type="submit" onClick={handleClick}>Show More</button> : null }
        {reviewModal ? <AddReview props={reviewModal} data={metaDataProps} close={setReviewModal} /> : null}
