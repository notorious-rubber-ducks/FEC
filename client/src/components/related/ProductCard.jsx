...
// pull in state from other components to handle click
  ...
  function handleProductCardClick() {
    ...
    Promise.all([
      axios.get(`${process.env.NODE_ENV === 'production' ? '/' : 'http://localhost:3000/'}products/${product.id}`)
        .then(({ data }) => data),
      axios.get(`${process.env.NODE_ENV === 'production' ? '/' : 'http://localhost:3000/'}products/${product.id}/styles`)
        .then(({ data }) => (data)),
    ])
      .then((values) => {
